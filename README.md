...
> The integration depends on the `aiounas` client library (`requirements` in the manifest). Until it is published to PyPI, install from source for development. See [`docs/API.md`](docs/API.md) for the API and [`docs/design/`](docs/design) / [`docs/plans/`](docs/plans) for the design.
